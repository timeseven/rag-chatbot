...
function Table({
	...
	return <TablePrimitive.Root className={cn('w-full caption-bottom text-sm', className)} {...props} />;
}
...
function TableHeader({
	...
	return <TablePrimitive.Header className={cn('border-border [&_tr]:border-b', className)} {...props} />;
}
...
function TableFooter({
	...
	return (
		<TablePrimitive.Footer className={cn('bg-muted/50 font-medium [&>tr]:last:border-b-0', className)} {...props} />
	);
}
...
function TableCell({
	...
	return <TablePrimitive.Cell className={cn('p-4 align-middle [&:has([role=checkbox])]:pr-0', className)} {...props} />;
}
